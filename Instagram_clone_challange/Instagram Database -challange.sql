...
WHO WON??!!*/
select t.photo_id ,t.image_url , u.username , t.no_of_likes from
(select l.photo_id ,p.user_id,p.image_url ,count(*) as no_of_likes from likes l 
left join photos p on l.photo_id = p.id group by l.photo_id ) as t 
left join users u 
on t.user_id = u.id order by t.no_of_likes desc limit 1 ;
...
GROUP BY users.id
HAVING comment_text IS NULL order by username;
...
select (count(no_comment.username)/count(u.username))*100 as '% users who never commented' ,
(count(u.username) - count(no_comment.username))*100/count(u.username) as '% users who commented at least on 1 post'
 from users u left join 
(select username from users where id not in (select user_id from comments ) order by username ) as no_comment
on  no_comment.username = u.username ;